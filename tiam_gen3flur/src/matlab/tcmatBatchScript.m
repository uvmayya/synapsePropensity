...
parfor job=1:length(batchSetupCell)
    tic
    ...
    if (exist(spotImgPath,'file')==2)
        spotImg = imread(spotImgPath);
    else
        %disp('No spot image file. Exiting');
        disp('No spot image file. Blank image used');
        spotImg=zeros(size(videocell{1})); 
        %return  can't use break with parfor
    end    
    ...
    saveFun(resultsFile,datacell,datacell2);
    ...
    halfCropSize=24; %crop-size for feature extractions; 
    % 12 works well for 30x on Olympus FV1200; 15 works well for 25x on LSM710; 20 works reasonable for 40x on
    % LSM510 but 22 is safe and for memory cells, it may be best to increase even further, say 26.
    ...
    if (exist(spotImgPath,'file')==2) % 3rd flur channel for spots
        datacell = get_flurInt(datacell, spotImg, halfCropSize, 0);
    end
    ...
    if outline_channel > 0
        if outline_channel == startimg, get_outline_new(datacell, videocell, halfCropSize, 4, params, pathToDir, expName);
        elseif outline_channel == startimg_irm, get_outline_new(datacell, videocell_irm, halfCropSize, 1, params, pathToDir, expName);
        elseif outline_channel == startimg_memory, get_outline_new(datacell, videocell_memory, halfCropSize, 2, params, pathToDir, expName);
        elseif outline_channel == startimg_naive, get_outline_new(datacell, videocell_naive, halfCropSize, 3, params, pathToDir, expName);
        end
    ...
    resultsFile = [pathToDir,'ws/', expName,'_results.mat'];
    saveFun(resultsFile,datacell,datacell2);%overwrites the previous one
...
% save and load function cannot be called under parfor, lest it results in 'Transperancy voilation error'
function saveFun(path, datacell, datacell2)
    save(path, 'datacell', 'datacell2');
end