...
function get_outline_new(datacell, videocell, halfCropSize, whichChannel, detectParams, pathToDir, expName)
...
for cp = 1 : size(datacell, 2)
	...
	for pos = 1 : size(datacell{cp}, 1)
        ...
        medianFrmInt=median(I(:)); %I(:) converts it to a column vector
        ...
        if whichChannel==4 %if DIC/transmitted channel
            ...
            Ie = edge(I, 'canny', 0.85*thresh, 0.8); %1.3*thresh for exp5; 0.85*thresh for FV1200
            ...
            % structural elements
            se90 = strel('line', 2, 90); % Willie had 3 in blob-check; perhaps 3 was used for the TIAM paper performance analysis
            se0 = strel('line', 2, 0); % Willie had 3 in blob-check; perhaps 3 was used for the TIAM paper performance analysis
            ...
            If = imfill(Id, 'holes');
            Ir = imerode(If, disk3); %willie had disk3
            img = imdilate(Ir, disk2); % willie had disk2
            centerBlob = imerode(Ir, disk3); % willie had disk2 and used img             
        ...
        if length(dist) == 1 %if there is only one component (background only) pick it
            if whichChannel==4 && overlap(1)<0.1
            ...
            elseif whichChannel==1 && mean2(I(imgLabel==1))+8 < medianFrmInt && overlap(1)<0.1
                boundary = bwperim(imgLabel,8);
            %elseif (whichChannel==2 || whichChannel==3) && mean2(I(imgLabel==1)) > medianFrmInt+8 && overlap(1)<0.1
            elseif (whichChannel==2 || whichChannel==3) && mean2(I(imgLabel==1)) > medianFrmInt && overlap(1)<0.1
                boundary = bwperim(imgLabel,8);
            end 
        elseif length(dist) >= 2 % if there are two or more components, pick the foreground component based on multiple conditions
            if whichChannel==4 % DIC
                ...
                for i=1:length(dist) % to pick based on proximity to the center of the cropped image
                    %if overlap(index(i))<0.1 && dist(index(i))<10 % closer to the center of the cropped image
                    if overlap(index(i))<0.1 && dist(index(i))<7 % closer to the center of the cropped image    
            ...
            elseif whichChannel==1 %IRM
                ...
                for i=1:length(dist) % consider the component based on larger area
                    ...
                    if overlap(index(i))<0.1 && mean2(I(imgLabel==index(i)))+8 < medianFrmInt && dist(index(i))<7    
            ...
            elseif whichChannel==2 || whichChannel==3 %flur
                ...
                for i=1:length(dist) % consider the component based on larger area
                    ...
                    if overlap(index(i))<0.1 && mean2(I(imgLabel==index(i))) > medianFrmInt && dist(index(i))<7    
...
% write the images with outline to a tiff file using the 'append' mode,
% which aborts sometimes when a batch (i.e. multiple analyses) job submission is done. 
...
% write the images as tiff files into a folder
if exist([pathToDir,'ws/',expName,'_outline'],'dir')==7  % 7 means the directory exists
    delete([pathToDir,'ws/',expName,'_outline/','*.tif']); %delete all existing tif files
else    
    mkdir([pathToDir,'ws/',expName,'_outline']); 
end  
for frame = 1 : size(videocell, 2)
    I=videocell{frame};
    tiffFileName=[pathToDir,'ws/', expName,'_outline/',int2str(frame),'.tif'];
    imwrite(I,tiffFileName) ;%,'tif','BitDepth',8); % append images into a tiff-series file
end
toc
