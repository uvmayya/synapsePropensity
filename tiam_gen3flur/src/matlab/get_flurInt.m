...
if whichFlurChannel~=0 && whichFlurChannel~=1 && whichFlurChannel~=2 
    disp('Incorrect flur channel specified')
    return
end   
...
for cp = 1:size(datacell,2)	
	for pos = 1:size(datacell{cp},1)
        ...
        if whichFlurChannel==0, I=videocell_flur; % spot image    
        else I=videocell_flur{frame}; end  % particular flur img frame  
        if center_x<=halfcropXsize
            halfcropXsize=center_x-1;
        elseif center_x+halfcropXsize>=size(I,2)
            halfcropXsize=size(I,2)-center_x; % column value corresponds to x-coordinate of the pixel
        end % to account for centroids that are too close the edges of the frames
        if center_y<=halfcropYsize
            halfcropYsize=center_y-1;
        elseif center_y+halfcropYsize>=size(I,1)
            halfcropYsize=size(I,1)-center_y; % row value corresponds to y-coordinate of the pixel
        end % to account for centroids that are too close the edges of the frames
        ...
        elseif whichFlurChannel==0
            datacell{cp}(pos,20) = meanFlurInt; % spot
        else
