...
for i = 1 : length(datacell)
    for j = i+1 : length(datacell) % the second track to be considered for joining can't be before the first track
        ...
        % sim begins equal to 0
        sim = 0; % changed from 1 to 0 on 7/2/15
...
for j=length(datacell):-1:1
   if max(joinmatrix(:,j))>joinThresh
       for i=j-1:-1:1
           if assig(i) == j && joinmatrix(i,j)>joinThresh %included the 2nd condition on 7/2/15; the if condition in the above for loop not sufficient
