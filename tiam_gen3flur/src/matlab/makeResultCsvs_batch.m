...
function makeResultCsvs_batch(datacell,resultDataType,vidName,irmOnly,params,arrestCoefThresh)
...
% input: vidName, string name of the video
...
if resultDataType == 0 || resultDataType == 2
	...
	for i = 1:length(datacell)
        ...
        %perCtMat(i,16) = mean(datacell{i}(:,18)); % 17 is eccentricity, 18 is circularity, 19 is aspect ratio as a polarity measure is reported
        %perCtMat(i,17) = mean(datacell{i}(:,19)); % 17 is eccentricity, 18 is circularity, 19 is aspect ratio as a polarity measure is reported
        perCtMat(i,16) = mean(datacell{i}(:,20)); % fluor 3/spot int value
    end

	% to save csvs
	nameLength = 20; % length of this function name "makeResultCsvs"
	...
	initSaveString = [vidName,'_perCellTrack'];
...
if resultDataType == 1 || resultDataType == 2
	...
	% ensure that csv files are not overwritten
	csvdirString = [pathToDir, '../../csv/'];
	initSaveString = [vidName,'_perFrame'];
	saveString = [initSaveString,'.csv'];
	for j=2:50
		alreadyInDir = isFileInDir(csvdirString,saveString);
		if alreadyInDir==0, break; end
